...
import { Search, Edit3, Trash2, Plus, X, Filter, AlertCircle, Save, BookOpen } from 'lucide-react';
import { useState, useEffect } from 'react';
import Layout from '@/Components/Layout/Layout';

export default function Asignaturas() {
    const { asignaturas, auth, error, filters: initialFilters, flash } = usePage().props;
    const user = auth?.user;
    
    const [search, setSearch] = useState(initialFilters?.search || '');
    const [filterActive, setFilterActive] = useState(initialFilters?.estado || 'todos');
    ...
    useEffect(() => {
        if (!search && filterActive === 'todos') return;
    ...
    const applyFilters = () => {
        router.get(route('secretaria.asignaturas'), {
        ...
        <Layout title="Gestión de Asignaturas">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mb-6">
                    <div className="bg-white rounded-lg shadow-md p-4">
                        ...
                        <p className="text-xl sm:text-2xl font-bold text-gray-900">{asignaturas?.total || 0}</p>
                    </div>
                    <div className="bg-white rounded-lg shadow-md p-4">
                        <p className="text-gray-500 text-xs sm:text-sm">Activas</p>
                        <p className="text-xl sm:text-2xl font-bold text-green-600">
                            {asignaturas?.data?.filter(a => a.is_active).length || 0}
                        </p>
                    </div>
                    <div className="bg-white rounded-lg shadow-md p-4">
                                        ...
                                        <tr key={asig.id} className="hover:bg-gray-50 transition">
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-500">
                                                <div className="max-w-xs truncate">
                                                    {asig.description || 'Sin descripción'}
                                                </div>
                                            </td>
                ...
                {/* Paginación */}
                {asignaturas?.links && asignaturas.links.length > 3 && (
                    <div className="mt-6 flex justify-center">
                        <div className="flex space-x-2">
                            {asignaturas.links.map((link, index) => (
                                <button
                                    key={index}
