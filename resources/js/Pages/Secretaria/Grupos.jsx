...
import { Search, Edit3, Trash2, Plus, X, Filter, AlertCircle, Save } from 'lucide-react';
import { useState, useEffect } from 'react';
import Layout from '@/Components/Layout/Layout';

export default function Grupos() {
    const { grupos, grados, cursos, auth, error, filters: initialFilters, flash } = usePage().props;
    const user = auth?.user;
    
    const [search, setSearch] = useState(initialFilters?.search || '');
    const [filterGrade, setFilterGrade] = useState(initialFilters?.grade_id || 'todos');
    const [filterCourse, setFilterCourse] = useState(initialFilters?.course_id || 'todos');
    ...
    // Auto-generar nombre del grupo cuando se escriben grado y curso
    useEffect(() => {
    ...
    useEffect(() => {
        if (!search && filterGrade === 'todos' && filterCourse === 'todos') return;
    ...
    const applyFilters = () => {
        router.get(route('secretaria.grupos'), {
            search: search || undefined,
            grade_id: filterGrade !== 'todos' ? filterGrade : undefined,
            course_id: filterCourse !== 'todos' ? filterCourse : undefined,
        }, { 
            preserveState: true,
            preserveScroll: true,
        });
        ...
        <Layout title="Gestión de Grupos">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                                        formErrors.grado_texto ? 'border-red-500' : 'border-gray-300'
                                    }`}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                                        formErrors.curso_texto ? 'border-red-500' : 'border-gray-300'
                                    }`}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mb-6">
                    <div className="bg-white rounded-lg shadow-md p-4">
                        ...
                        <p className="text-xl sm:text-2xl font-bold text-gray-900">{grupos?.total || 0}</p>
                    </div>
                    <div className="bg-white rounded-lg shadow-md p-4">
                        <p className="text-gray-500 text-xs sm:text-sm">Grados Únicos</p>
                        <p className="text-xl sm:text-2xl font-bold text-blue-600">
                            {new Set(grupos?.data?.map(g => g.grade_id).filter(Boolean)).size}
                        </p>
                    </div>
                    <div className="bg-white rounded-lg shadow-md p-4">
                        <p className="text-gray-500 text-xs sm:text-sm">Cursos Únicos</p>
                        <p className="text-xl sm:text-2xl font-bold text-purple-600">
                            {new Set(grupos?.data?.map(g => g.course_id).filter(Boolean)).size}
                        </p>
                    </div>
                </div>
