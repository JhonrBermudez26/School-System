...
import {
  ...
  Filter, ArrowUpDown, ArrowUp, ArrowDown, Calendar
} from 'lucide-react';
...
export default function RegistrarNotas() {
  ...
  return (
    <Layout title="Registrar Notas">
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 -m-4 sm:-m-6 md:-m-8 p-4 sm:p-6 lg:p-8">
        <div className="max-w-[1600px] mx-auto space-y-4 sm:space-y-6">
          {/* Header */}
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl overflow-hidden border border-gray-100">
            <div className="bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600 relative overflow-hidden px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                    <div className="relative">
                      <select
                        ...
                        className="w-full px-4 py-2.5 sm:py-3 rounded-xl border-2 border-white/30 bg-white/20 text-white focus:border-white focus:ring-4 focus:ring-white/30 transition-all appearance-none pr-10 text-sm sm:text-base"
                      >
                        <option value="" className="text-gray-900">Materia...</option>
                        {Array.from(new Set(asignaciones.map(a => a.subject_id))).map(subjectId => {
                          const asig = asignaciones.find(a => a.subject_id === subjectId);
                          return <option key={subjectId} value={subjectId} className="text-gray-900">{asig.subject_name}</option>;
                        })}
                      </select>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full px-4 py-2.5 sm:py-3 rounded-xl border-2 border-white/30 bg-white/20 text-white focus:border-white focus:ring-4 focus:ring-white/30 transition-all appearance-none pr-10 disabled:bg-white/10 disabled:text-white/50 disabled:cursor-not-allowed text-sm sm:text-base"
                      >
                        <option value="" className="text-gray-900">Grupo...</option>
                        {availableGroups.map(asig => (
                          <option key={asig.group_id} value={asig.group_id} className="text-gray-900">{asig.group_name}</option>
                        ))}
                      </select>
                      <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-white pointer-events-none" />
                    </div>
                    <button
                      ...
                      className="px-6 py-2.5 sm:py-3 bg-white text-blue-700 rounded-xl hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg font-bold text-sm sm:text-base"
                    >
                      Cargar
                    </button>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 border border-gray-100">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 sm:mb-6">
                  <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold text-gray-900">{classInfo.subject_name}</h2>
                      <p className="text-sm sm:text-base text-gray-600">Grupo {classInfo.group_name}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setShowManualGradeForm(true)}
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all shadow-lg font-bold text-sm sm:text-base"
                  >
                      ...
                      <table className="w-full border-collapse table-fixed">
                        <thead className="sticky top-0 z-30">
                          <tr className="bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600">
                            <th className="w-[200px] px-4 py-4 text-left border-r border-white/20 bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600">
                              <button
                            ...
                            <th className="w-[120px] px-3 py-4 text-left border-r border-white/20 bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600">
                              <button
                            ...
                            {allGradeColumns.map((col) => (
                              <th 
                                key={col.key} 
                                className="px-2 py-4 text-center text-xs font-bold text-white border-r border-white/20 bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600"
                            ...
                            <th className="w-[100px] px-3 py-4 text-center bg-gradient-to-r from-indigo-600 to-blue-600 border-l border-white/20">
                              <button
          ...
          {!classInfo && (
            <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl p-8 sm:p-12 lg:p-16 text-center border border-gray-100">
              <div className="w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-blue-100 to-indigo-200 rounded-full flex items-center justify-center mx-auto mb-6">
                <ClipboardList className="w-10 h-10 sm:w-12 sm:h-12 text-blue-600" />
              </div>
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Selecciona una clase</h3>
...
function StatCard({ icon: Icon, label, value, color, className = '' }) {
  ...
  return (
    <div className={`bg-gradient-to-br ${gradientClass} rounded-xl p-3 sm:p-4 border ${borderClass} ${className}`}>
      <div className="flex items-center gap-2 mb-2">
        <Icon className={`w-4 h-4 ${iconClass}`} />
        <div className={`text-xs font-semibold ${textClass}`}>{label}</div>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-4">
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            <h3 className="text-white font-bold text-lg">{student.student_name}</h3>
            <p className="text-white/80 text-sm">Doc: {student.document_number || 'N/A'}</p>
          </div>
          <div className="text-right">
            <div className="text-white/80 text-xs mb-1">Promedio</div>
            <div className={`text-2xl font-bold ${student.average >= 3.0 ? 'text-green-300' : student.average > 0 ? 'text-orange-300' : 'text-white/50'}`}>
              {student.average ? student.average.toFixed(1) : '-'}
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full mt-2 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-white text-sm font-medium flex items-center justify-center gap-2 transition-colors"
        >
          {expanded ? (
            <>
          ...
          {allGradeColumns.map((col) => {
            ...
            return (
              <div key={col.key} className="border-b border-gray-200 pb-3 last:border-0">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <div className="font-semibold text-gray-900 text-sm">{col.title}</div>
...
function EmptyState({ onCreateClick }) {
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 sm:p-12 text-center border border-gray-100">
      <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-blue-100 to-indigo-200 rounded-full flex items-center justify-center mx-auto mb-6">
        <ClipboardList className="w-8 h-8 sm:w-10 sm:h-10 text-blue-600" />
      </div>
      <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Sin evaluaciones registradas</h3>
      <p className="text-sm sm:text-base text-gray-600 mb-6">
        No hay tareas ni evaluaciones manuales para este grupo aún.<br />
        Crea tareas desde "Mis clases" o agrega una evaluación manual aquí.
      </p>
      <button
        onClick={onCreateClick}
        className="inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all shadow-lg font-bold text-sm sm:text-base"
      >
        <Plus className="w-4 h-4 sm:w-5 sm:h-5" />
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-blue-500 to-indigo-600 px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/20 rounded-xl flex items-center justify-center">
              <Plus className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            </div>
            <h2 className="text-xl sm:text-2xl font-bold text-white">Nueva Evaluación Manual</h2>
          </div>
          <button 
            onClick={onClose} 
        ...
        <form onSubmit={onSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-5">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 sm:py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all text-sm sm:text-base"
              required
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-2.5 sm:py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all resize-none text-sm sm:text-base"
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
            <button
              type="button"
              onClick={onClose}
              className="w-full sm:flex-1 px-6 py-2.5 sm:py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition-all font-bold text-sm sm:text-base"
              disabled={saving}
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={saving}
              className="w-full sm:flex-1 px-6 py-2.5 sm:py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all shadow-lg font-bold disabled:opacity-50 text-sm sm:text-base"
            >
              {saving ? (
