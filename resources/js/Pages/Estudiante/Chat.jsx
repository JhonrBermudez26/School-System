...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        const res = await axios.get(route('estudiante.chat.search'), {
  ...
  const markAsRead = async (conversationId) => {
    try {
      await axios.post(route('estudiante.chat.read', conversationId));
    } catch (e) {
  ...
  const leaveGroup = async () => {
    ...
    try {
      const response = await axios.post(
        route('estudiante.chat.leave', selectedConversation.id)
      );
