...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        const res = await axios.get(route('estudiante.chat.search'), { params: { query: userSearchQuery } });
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      setIsSearching(true);
      try {
        const res = await axios.get(route('estudiante.chat.search'), { params: { query: searchQuery } });
  ...
  const fetchConversations = async () => {
    try {
      const res = await axios.get(route('estudiante.chat.conversations.json'));
      setConversations(res.data);
    } catch (error) { console.error('Error cargando conversaciones:', error); }
  };

  const markAsRead = async (conversationId) => {
    try { await axios.post(route('estudiante.chat.read', conversationId)); }
  ...
  const startPersonalChat = async (userId) => {
    try {
      const response = await axios.post(route('estudiante.chat.create'), {
      ...
      if (response.data?.conversation_id) {
        ...
        if (conv) {
          ...
          const convResponse = await axios.get(
            route('estudiante.chat.show', response.data.conversation_id),
  ...
  const selectConversation = async (conv) => {
    ...
    try {
      const res = await axios.get(route('estudiante.chat.show', conv.id), {
        headers: { Accept: 'application/json' }
      });
  ...
  // ✅ handleEditMessage como función separada (igual que profesor)
  const handleEditMessage = async () => {
    if (!editingMessage) return;
    try {
      await axios.put(
        route('estudiante.chat.edit-message', editingMessage.id),
        { body: editedText }
      );
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await axios.post(
        route('estudiante.chat.message', selectedConversation.id),
        formData
      );
  ...
  const updateGroupInfo = () => {
    ...
    router.put(route('estudiante.chat.update-group', selectedConversation.id), {
  ...
  const addParticipantToGroup = async (userId) => {
    ...
    try {
      const response = await axios.post(
        route('estudiante.chat.addParticipant', selectedConversation.id),
        { user_id: userId }
      );
  ...
  const leaveGroup = async () => {
    ...
    try {
      const response = await axios.post(route('estudiante.chat.leave', selectedConversation.id));
      if (response.data.success) {
  ...
  const handleDeleteMessage = async () => {
    ...
    try {
      await axios.delete(route('estudiante.chat.delete-message', selectedMessage.id), {
  ...
  const handleDeleteChat = async () => {
    if (!selectedConversation) return;
    try {
      await axios.delete(route('estudiante.chat.delete-conversation', selectedConversation.id));
  ...
  // ✅ startEdit carga editedText Y newMessage correctamente (igual que profesor)
  const startEdit = () => {
            ...
            {selectedConversation ? (
              <>
                ...
                {showMessageMenu && selectedMessage && (
                  ...
                  >
                    {/* ✅ Igual que profesor: editar y copiar solo si es texto propio y no eliminado */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 relative">
                      <input
                        ...
                        // ✅ Igual que profesor: editedText cuando edita, newMessage cuando envía
                        ...
                        onKeyDown={e => {
                          if (e.key === 'Enter' && !e.shiftKey) {
                            e.preventDefault();
                            // ✅ Igual que profesor: llama handleEditMessage correctamente
                    ...
                    {/* ✅ Igual que profesor: botón enviar llama handleEditMessage */}
