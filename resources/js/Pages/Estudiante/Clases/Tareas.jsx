...
export default function Tareas({ tasks: initialTasks = [], classInfo }) {
  ...
  const loadTasks = async () => {
    try {
      const response = await fetch(
        `/estudiante/clases/tasks?subject_id=${classInfo.subject_id}&group_id=${classInfo.group_id}`,
        {
  ...
  const handleShowSubmit = (task, isEdit = false) => {
    ...
    setSubmissionComment(task.submission?.comment || '');
    setFiles([]);
    setSelectedMembers(task.submission?.members?.map(m => m.student_id) || []);
    
    // Si no es individual, cargar compañeros disponibles
    if (task.work_type !== 'individual' && !isEdit) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedTask.work_type !== 'individual' && !isEditing) {
  ...
  const MemberSelector = () => (
    <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6 border-2 border-purple-200">
      <h3 className="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2">
        ...
        Selecciona compañeros para trabajar
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="font-semibold text-gray-900">{classmate.name}</p>
                  <p className="text-xs text-gray-600">{classmate.email}</p>
                </div>
              </div>
              {selectedMembers.includes(classmate.id) && (
          ...
          <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl max-h-[92vh] overflow-hidden flex flex-col border border-gray-200">
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-6">
              ...
              <div>
                ...
                <div className="prose prose-sm max-w-none text-gray-700 bg-gray-50 rounded-xl p-5 border border-gray-200 whitespace-pre-wrap">
              ...
              {selectedTask.attachments?.length > 0 && (
                <div>
                  <h3 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2">
                    ...
                    Archivos de la tarea ({selectedTask.attachments.length})
                  </h3>
                  <div className="space-y-2">
                    {selectedTask.attachments.map((att) => (
                      <button
                        ...
                        className="flex items-center gap-3 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200 hover:from-blue-100 hover:to-indigo-100 transition-all w-full text-left group"
                      >
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border-2 border-blue-200">
                  ...
                  {selectedTask.submission.members?.length > 0 && (
                    <div className="mb-4 bg-white rounded-xl p-4 border border-blue-100">
                      <p className="text-xs font-bold text-gray-700 mb-3">Trabajo en {selectedTask.work_type === 'pairs' ? 'pareja' : 'grupo'}:</p>
                      <div className="space-y-2">
                        {selectedTask.submission.members.map((member) => (
                          <div key={member.id} className="flex items-center justify-between p-2 bg-blue-50 rounded-lg">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm font-medium text-gray-900">{member.student_name}</span>
                              {member.is_creator && (
                  ...
                  {selectedTask.submission.comment && (
                    <div className="bg-white rounded-xl p-4 mb-4 border border-blue-100">
                      ...
                      <p className="text-sm text-gray-700 whitespace-pre-wrap">{selectedTask.submission.comment}</p>
                    </div>
                  )}
                  ...
                  {selectedTask.submission.files?.length > 0 && (
                    <div className="space-y-2">
                      ...
                      {selectedTask.submission.files.map((file) => (
                        <div key={file.id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-blue-100">
                          <button
                            onClick={() => downloadFile(file.file_path, file.file_name)}
                            className="flex items-center gap-3 flex-1 hover:bg-blue-50 transition-all text-left group"
                          >
                            <FileText className="h-4 w-4 text-blue-600" />
                            <span className="text-sm font-medium text-gray-900 flex-1 truncate">{file.file_name}</span>
                  ...
                  {selectedTask.submission.teacher_feedback && (
                    <div className="mt-4 bg-yellow-50 rounded-xl p-4 border-2 border-yellow-200">
                      ...
                      <p className="text-sm text-gray-700 whitespace-pre-wrap">{selectedTask.submission.teacher_feedback}</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="px-6 sm:px-8 py-5 border-t bg-gray-50 flex justify-end gap-3">
              ...
              {(!selectedTask.submission || selectedTask.submission.status === 'pending') && !selectedTask.is_closed && (
                <button
                  ...
                  className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all flex items-center gap-2"
                >
              ...
              {canEdit && (
                <button
                  ...
                  className="px-8 py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all flex items-center gap-2"
                >
      ...
      <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl max-h-[92vh] overflow-hidden flex flex-col border border-gray-200">
            <div className="px-6 sm:px-8 py-5 border-b bg-gradient-to-r from-blue-50 to-indigo-50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center">
                  {isEditing ? <Edit className="h-6 w-6 text-white" /> : <Upload className="h-6 w-6 text-white" />}
                </div>
                <div>
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-6">
              {/* Selector de miembros (solo si no es individual y no está editando) */}
              {selectedTask.work_type !== 'individual' && !isEditing && (
                <MemberSelector />
              )}
              ...
              <div>
                ...
                <textarea
                  className="w-full px-5 py-3.5 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 resize-none transition-all"
                  rows={6}
              ...
              {/* Archivos existentes (si está editando) */}
              {isEditing && selectedTask.submission?.files?.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedTask.submission.files.map((file) => (
                      <div key={file.id} className="flex items-center justify-between p-4 bg-blue-50 rounded-xl border border-blue-200">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="px-6 sm:px-8 py-5 border-t bg-gray-50 flex flex-col-reverse sm:flex-row justify-end gap-3">
              ...
              >
                {submitting ? (
                  ...
                  <>
                    {isEditing ? <Save className="h-5 w-5" /> : <Send className="h-5 w-5" />}
                    {isEditing ? 'Actualizar entrega' : 'Enviar tarea'}
                  </>
                )}
              </button>
            </div>
      ...
      <div className="space-y-5">
        ...
        {filteredTasks.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-xl p-12 text-center border border-gray-100">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">No hay tareas en esta categoría</h3>
                  ...
                  <div className="p-5 sm:p-7">
                        ...
                        <div className="flex flex-wrap items-center gap-3 mb-3">
                          ...
                          {task.submission?.members?.length > 0 && (
                            <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-xs font-bold flex items-center gap-1">
                              ...
                              {task.work_type === 'pairs' ? 'En pareja' : 'En grupo'}
                            </span>
                          )}
                        </div>
                        <p className="text-gray-600 line-clamp-2 leading-relaxed">{task.description}</p>
                      </div>
                      <button
                        onClick={() => handleViewTask(task)}
                        className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all whitespace-nowrap shadow-lg hover:shadow-xl flex items-center gap-2"
                      >
                    ...
                    {(!task.submission || task.submission.status === 'pending') && !task.is_closed && (
                      <button
                        ...
                        className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-3.5 rounded-xl font-bold hover:from-blue-600 hover:to-indigo-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2"
                      >
                        <Upload className="h-5 w-5" />
                        Entregar tarea
                      </button>
...
//implementar logica para editar la entrega.
// implementar logica de calificacion.