...
// ─────────────────────────────────────────────────────────────────────────────
// CONSTANTES DE DÍAS
// Los valores internos coinciden EXACTAMENTE con el enum de la BD:
//   ['Lunes','Martes','Miercoles','Jueves','Viernes']  ← sin acento
// dayLabels se usa SOLO para mostrar texto en la UI.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function Horarios() {
    ...
    // ── SLOTS OCUPADOS (TODOS LOS GRUPOS) ────────────────────────────────
    ...
    // ── ASIGNATURAS DISPONIBLES PARA UN SLOT ─────────────────────────────
    ...
    const filteredSubjects = useMemo(() => {
        let list = [];
        if (addingCell) {
            list = getAvailableSubjectsForSlot(addingCell.day, addingCell.timeSlotId);
        } else if (editingCell) {
            list = getAvailableSubjectsForSlot(editingCell.day, editingCell.timeSlotId, editingCell.subject_id);
        }
        if (searchTerm && list.length > 0) {
            return list.filter(a => a.subject_name.toLowerCase().includes(searchTerm.toLowerCase()));
        }
        return list;
    ...
    // ── SCROLL AL HORARIO DESPUÉS DE NAVEGAR ─────────────────────────────
    useEffect(() => {
    ...
    const handleAddSubject = async (subjectId, userId) => {
        ...
        await router.post(route('coordinadora.horarios.add-slot'), {
            ...
            day:          addingCell.day,
            time_slot_id: addingCell.timeSlotId,
            ...
            onError:   (e) => console.error('Error al agregar:', e),
    ...
    const handleDragStart = (e, day, timeSlotId, cell) => {
        ...
        e.dataTransfer.setData('text/plain', JSON.stringify({ day, timeSlotId }));
    };

    const handleDragOver = (e, day, timeSlotId) => {
        if (!can.update || data.mode !== 'group') return;
        e.preventDefault();
        if (canDropHere(day, timeSlotId)) {
    ...
    const handleDrop = async (e, targetDay, targetTimeSlotId) => {
        ...
        if (sourceDay === targetDay && sourceTimeSlotId === targetTimeSlotId) {
        ...
        await router.post(route('coordinadora.horarios.move'), {
            ...
            source_time_slot_id:  sourceTimeSlotId,
            ...
            onError:   (e) => console.error('Error al mover:', e),
            onFinish:  () => setIsLoading(false),
        });
    };
    ...
    const handleEditSlot = (day, timeSlotId, cell) => {
        if (!can.update || data.mode !== 'group') return;
        setAddingCell(null);
        setEditingCell({ day, timeSlotId, subject_id: cell.subject_id });
        setSearchTerm('');
    };
    ...
    const handleChangeSubject = async (newSubjectId) => {
        ...
        await router.put(route('coordinadora.horarios.update-slot'), {
            group_id:     data.group_id,
            day:          editingCell.day,
            time_slot_id: editingCell.timeSlotId,
    ...
    const handleDeleteSlot = async (day, timeSlotId) => {
        ...
        await router.delete(route('coordinadora.horarios.delete-slot'), {
            ...
            preserveState: false,
            preserveScroll: true,
            onError:  (e) => console.error('Error al eliminar:', e),
            onFinish: () => setIsLoading(false),
        });
    };
    ...
    return (
        <Layout title="Gestión de Horarios">
            <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {flash?.success && (
                    <div className="p-4 bg-green-50 border-l-4 border-green-500 text-green-700 rounded-xl">
                        ✅ {flash.success}
                    </div>
                )}
                {flash?.error && (
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-teal-50 rounded-xl shadow-md p-5 border border-green-100">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <button
                                ...
                                onClick={() => {
                                    if (confirm('¿Generar horarios para todos los grupos? Esto puede tomar unos segundos.')) {
                            ...
                            <button
                                ...
                                className="px-5 py-2.5 bg-gradient-to-r from-orange-500 to-amber-600 text-white rounded-lg font-medium shadow hover:from-orange-600 hover:to-amber-700 transition-all hover:scale-105 disabled:opacity-50"
                            >
                                Regenerar
                            </button>
                        </div>
                ...
                <div className="space-y-6">
                    <div className="flex justify-center">
                        <div className="inline-flex rounded-full bg-gray-100 p-1 shadow-inner">
                            ...
                            >
                                <Users className="inline h-4 w-4 mr-1.5" /> Grupos
                            </button>
                            <button
                            ...
                            >
                                <User className="inline h-4 w-4 mr-1.5" /> Docentes
                            </button>
                        </div>
                    </div>
                        ...
                        <div className="p-5 space-y-5">
                            {time_slots.map((slot) => {
                                const daySlots = DAYS.map(day => ({
                                    day,
                                    cell: data.mode === 'group'
                                        ? grid[day]?.[slot.id]
                                        : teacherGrid[day]?.[slot.id],
                                }));
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`min-h-[100px] transition-all ${
                                                            isDragOver ? 'ring-2 ring-green-400 bg-green-50 scale-105' : ''
                                                        }`}
                                                    >
                                                        {/* Nombre del día — usa DAY_LABELS para mostrar acento */}
                                                        ...
                                                        {isAdding ? (
                                                            /* ── MODO AGREGAR ── */
                                                            <div className="bg-purple-50 p-2 rounded-lg border-2 border-purple-400 max-h-60 overflow-y-auto">
                                                                <div className="relative mb-2">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        className="w-full text-xs px-6 py-1 border rounded"
                                                                        autoFocus
                                                                    />
                                                                </div>
                                                                <div className="space-y-1 max-h-40 overflow-y-auto">
                                                                    {filteredSubjects.length > 0 ? (
                                                                        filteredSubjects.map(asg => (
                                                                            <button
                                                                                key={asg.subject_id}
                                                                                onClick={() => handleAddSubject(asg.subject_id, asg.user_id)}
                                                                                className="w-full text-left px-2 py-1 text-xs bg-white hover:bg-purple-100 rounded border border-purple-200 transition-colors"
                                                                            >
                                                                                <div className="font-semibold truncate">{asg.subject_name}</div>
                                                                                <div className="text-gray-600 text-[10px] truncate">
                                                                                    {asg.teacher_name} {asg.teacher_last_name}
                                                                                </div>
                                                                            </button>
                                                                        ))
                                                                    ) : (
                                                                        <p className="text-xs text-gray-500 py-2">
                                                                            {available_assignments.length === 0
                                                                                ? 'No hay asignaturas asignadas al grupo'
                                                                                : 'No hay profesores disponibles en este horario'}
                                                                        </p>
                                                                    )}
                                                                </div>
                                                                <button
                                                                    onClick={() => { setAddingCell(null); setSearchTerm(''); }}
                                                            ...
                                                            <div className="bg-blue-50 p-2 rounded-lg border-2 border-blue-400 max-h-60 overflow-y-auto">
                                                                ...
                                                                <div className="space-y-1 max-h-40 overflow-y-auto">
                                                                    {filteredSubjects.length > 0 ? (
                                                                        filteredSubjects.map(asg => (
                                                                            <button
                                                                                key={asg.subject_id}
                                                                                onClick={() => handleChangeSubject(asg.subject_id)}
                                                                ...
                                                                <button
                                                                    onClick={() => { setEditingCell(null); setSearchTerm(''); }}
                                                                    className="mt-2 w-full bg-gray-400 text-white px-2 py-1 rounded text-xs hover:bg-gray-500"
                                                                >
                                                                    <X className="h-3 w-3 inline mr-1" /> Cancelar
                                                                </button>
                                                            ...
                                                            <div
                                                                ...
                                                                className={`group/cell relative bg-green-50 p-3 rounded-lg text-sm transition-all ${
                                                                    can.update && data.mode === 'group'
                                                            ...
                                                            >
                                                                ...
                                                                {can.update && can.delete && data.mode === 'group' && (
                                                                    <div className="absolute top-1 right-1 opacity-0 group-hover/cell:opacity-100 transition-opacity flex gap-1">
                                                                        <button
                                                                            onClick={(e) => { e.stopPropagation(); handleEditSlot(day, slot.id, cell); }}
                                                                        ...
                                                                        <button
                                                                            onClick={(e) => { e.stopPropagation(); handleDeleteSlot(day, slot.id); }}
                                                            ...
                                                            <button
                                                                ...
                                                                className={`w-full h-full text-gray-300 text-xl py-4 rounded-lg transition-all ${
                                                                    can.create && data.mode === 'group'
                                                                        ? 'hover:bg-purple-50 hover:text-purple-400 hover:border-2 hover:border-dashed hover:border-purple-300 cursor-pointer group/add'
                                                                        : ''
                                                                }`}
                                                            >
                                                                {can.create && data.mode === 'group' ? (
