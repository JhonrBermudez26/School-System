...
class TaskController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $totalStudents = DB::table('group_user as gu')
            ...
            ->distinct()->count('gu.user_id');

        $query = Task::with(['attachments', 'submissions.student', 'academicPeriod'])
            ->where('subject_id', $subjectId)->where('group_id', $groupId)->where('teacher_id', auth()->id());

        if ($periodId) $query->where('academic_period_id', $periodId);

        $tasks = $query->orderBy('created_at', 'desc')->get()->map(function ($task) use ($totalStudents) {
            $submitted = $task->submissions()->where('status', '!=', 'pending')->count();
            $graded    = $task->submissions()->where('status', 'graded')->count();
            ...
            return [
                ...
                'academic_period'      => $task->academicPeriod ? ['id' => $task->academicPeriod->id, 'name' => $task->academicPeriod->name] : null,
                'stats'                => ['total' => $totalStudents, 'submitted' => $submitted, 'graded' => $graded, 'pending' => $totalStudents - $submitted],
                'can'                  => ['update' => auth()->user()->can('update', $task), 'delete' => auth()->user()->can('delete', $task), 'grade' => auth()->user()->can('grade', $task)],
    ...
    public function store(Request $request)
    {
        ...
        if (!isset($validated['academic_period_id'])) {
            $validated['academic_period_id'] = AcademicPeriod::getPeriodoActual()?->id;
        }

        if (!$this->validatePeriodEnabled($validated['academic_period_id'])) {
            return response()->json(['message' => 'El periodo académico seleccionado no permite la carga de notas.', 'error' => 'period_disabled'], 403);
        }
        ...
        try {
            ...
            $task = Task::create([
                ...
                'close_date'           => !empty($validated['close_date']) ? Carbon::parse($validated['close_date'])->setTimezone('America/Bogota') : null,
            ...
            $students = DB::table('group_user as gu')
                ->join('model_has_roles as mhr', fn($j) => $j->on('gu.user_id', '=', 'mhr.model_id')->where('mhr.model_type', '=', 'App\\Models\\User'))
                ->join('roles as r', 'mhr.role_id', '=', 'r.id')
                ->where('gu.group_id', $validated['group_id'])->where('r.name', 'estudiante')
                ->pluck('gu.user_id');

            foreach ($students as $studentId) {
                TaskSubmission::firstOrCreate(['task_id' => $task->id, 'student_id' => $studentId], ['status' => 'pending', 'is_late' => false]);
            }

            if ($request->hasFile('attachments')) {
                foreach ($request->file('attachments') as $file) {
                    $path = $file->store('tasks/' . $task->id, 'private');
                    TaskAttachment::create(['task_id' => $task->id, 'file_name' => $file->getClientOriginalName(), 'file_path' => $path, 'file_type' => $file->getClientMimeType(), 'file_size' => $file->getSize()]);
                }
            }
            ...
            return response()->json(['message' => 'Tarea creada exitosamente', 'task' => $task->load('attachments', 'academicPeriod')], 201);
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Error creando tarea:', ['error' => $e->getMessage()]);
            return response()->json(['message' => 'Error al crear la tarea', 'error' => config('app.debug') ? $e->getMessage() : 'Error interno'], 500);
        }
    }

    /**
     * ✅ FIX IDOR: Task $task usa Route Model Binding + TaskPolicy::update() verifica teacher_id
     */
    public function update(Request $request, Task $task)
    {
        ...
        if (isset($validated['academic_period_id']) && !$this->validatePeriodEnabled($validated['academic_period_id'])) {
            return response()->json(['message' => 'El periodo académico seleccionado no permite operaciones.', 'error' => 'period_disabled'], 403);
        }
        ...
        try {
            ...
            $updates = $validated;
            if (isset($validated['due_date'])) $updates['due_date'] = Carbon::parse($validated['due_date'])->setTimezone('America/Bogota');
            if (isset($validated['close_date'])) $updates['close_date'] = !empty($validated['close_date']) ? Carbon::parse($validated['close_date'])->setTimezone('America/Bogota') : null;
            ...
            return response()->json(['message' => 'Tarea actualizada exitosamente', 'task' => $task->load('attachments', 'academicPeriod')]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['message' => 'Error al actualizar la tarea', 'error' => config('app.debug') ? $e->getMessage() : 'Error interno'], 500);
        }
    }
    ...
    public function destroy(Task $task)
    {
        ...
        try {
            ...
            DB::rollBack();
            return response()->json(['message' => 'Error al eliminar la tarea', 'error' => config('app.debug') ? $e->getMessage() : 'Error interno'], 500);
        }
    }
    ...
    public function show(Task $task)
    {
        ...
        if ($task->work_type === 'individual') {
            $submissions = TaskSubmission::with(['student', 'files'])->where('task_id', $task->id)->get();
        } else {
            $submissions = TaskSubmission::with(['student', 'files', 'members.student'])
                ->where('task_id', $task->id)
                ->whereNotExists(fn($q) => $q->select(DB::raw(1))->from('task_submission_members')
        ...
        $stats = [
            ...
            'submitted' => TaskSubmission::where('task_id', $task->id)->whereIn('status', ['submitted', 'graded'])->count(),
            'graded'    => TaskSubmission::where('task_id', $task->id)->where('status', 'graded')->count(),
            'pending'   => TaskSubmission::where('task_id', $task->id)->where('status', 'pending')->count(),
        ];
