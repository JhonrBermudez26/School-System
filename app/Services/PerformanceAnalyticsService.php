...
class PerformanceAnalyticsService
{
     ...
     * 
     * El modelo Attendance NO tiene academic_period_id.
     * Filtramos por las fechas del periodo usando start_date y end_date,
     * y por los grupos del estudiante para acotar correctamente.
     */
    protected function getAbsenceRate($studentId, $periodId)
    {
        ...
        // Total de días de clase registrados en el periodo (cualquier estudiante)
        // usando las fechas del periodo como rango, columna 'date'
        $totalDays = Attendance::whereBetween('date', [
        ...
        // Días en que este estudiante estuvo ausente en el rango del periodo
        $absentDays = Attendance::where('user_id', $studentId)
            ->whereBetween('date', [
                $period->start_date->format('Y-m-d'),
                $period->end_date->format('Y-m-d'),
            ])
        ...
        return round(($absentDays / $totalDays) * 100, 2);
    }
    /**
     * KPIs vacíos
     */
